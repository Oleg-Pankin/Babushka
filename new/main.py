...
class MyApp(App):
    def build(self):
        ...
        button = Button(text = 'Я соглашаюсь с тем что сам отвичаю за свою жизнь.',
                        text_size=(300, None),
                        background_color = random.choice(colors))
        ...
        txt = Label(text='Мама была права. Всё еще точно не известно, почему куриный бульон заставляет нас чувствовать'
                         ...
                         ' симптомов простуды.', text_size=(400, None))
        hl5.add_widget(txt)
        hl5.add_widget(img)
        hl5.add_widget(button)
        hl6 = BoxLayout(orientation="vertical")
        screenmenu6.add_widget(hl6)
        button = Button(text='Обратно')
        button.bind(on_press=self.on_press_button7)
        img = Image(source='Advice4.jpg')
        txt = Label(text='Они не только хороши для организма — свежие хрустящие фрукты и овощи также помогут сохранить'
                         ' ваши зубы жемчужно-белыми. Во время жевания яблок и моркови их мелкие частицы очищают'
                         ' поверхность зубов, помогая удалить пятна. Яблоки и клубника содержат яблочную кислоту,'
                         ' которая помогает отбелить эмаль.', text_size=(300, None))
        hl6.add_widget(txt)
        hl6.add_widget(img)
        hl6.add_widget(button)
        hl7 = BoxLayout(orientation="vertical")
        screenmenu7.add_widget(hl7)
        button = Button(text='Обратно')
        button.bind(on_press=self.on_press_button7)
        img = Image(source='Advice5.jpg')
